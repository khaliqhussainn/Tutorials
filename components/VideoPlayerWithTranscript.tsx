  ...
  (
    ...
    const fetchTranscript = async () => {
      try {
        ...
        // Use the public API endpoint instead of admin endpoint
        ...
        if (response.ok) {
          ...
          if (data.hasTranscript && data.transcript) {
            ...
            console.log("Transcript loaded:", segments.length, "segments"); // Debug log
          } else {
            console.log("No transcript available, status:", data.status); // Debug log
            setTranscript(null);
          }
        } else {
          const errorData = await response.json();
          console.error("Failed to fetch transcript:", errorData.error);
        }
      } catch (error) {
    ...
    return (
      <div
        className={`grid ${
          showTranscript ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"
        } gap-6`}
      >
        {/* Video Player */}
