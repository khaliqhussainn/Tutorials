// components/admin/EnhancedVideoUpload.tsx
'use client'
...
export default function EnhancedVideoUpload({ 
  ...
  const [progress, setProgress] = useState<UploadProgress>({ upload: 0, transcript: 0 })
    ...
    try {
      ...
      const createVideoResponse = await fetch('/api/admin/videos', {
        ...
        body: JSON.stringify({
          ...
          order: 1,
          aiPrompt: videoData.aiPrompt.trim() || null
        })
      })
      ...
      // Step 3: Generate transcript if enabled
      if (autoTranscript) {
        setCurrentStep('transcript')
        setProgress(prev => ({ ...prev, transcript: 10 }))

        const transcriptResponse = await fetch(`/api/admin/videos/${createdVideo.id}/transcript`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        })
        ...
        if (transcriptResponse.ok) {
          ...
          console.log('Transcript generated successfully:', transcriptResult)
        } else {
          console.warn('Transcript generation failed, but video was created successfully')
        }
      }
  ...
  const resetForm = () => {
    setFile(null)
    setVideoData({ title: '', description: '', aiPrompt: '' })
    setProgress({ upload: 0, transcript: 0 })
    setCurrentStep('select')
    setError(null)
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-gray-500">
                ...
                {currentStep === 'transcript' && 'Generating transcript...'}
              ...
              {autoTranscript && (
                <div className="flex items-center space-x-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                    ...
                    getStepStatus('transcript') === 'current' ? 'bg-blue-500' : 'bg-gray-300'
                  }`}>
            ...
            <div className="flex items-center p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-green-600">
                  {autoTranscript ? 'Video and transcript are ready' : 'Video is ready for viewing'}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              ...
              <div>
                ...
                <textarea
                  value={videoData.aiPrompt}
                  onChange={(e) => setVideoData(prev => ({ ...prev, aiPrompt: e.target.value }))}
                  placeholder="Describe what students will learn in this video (helps generate better quiz questions)"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  rows={2}
                  maxLength={500}
                />
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-blue-900">Auto-generate Transcript</p>
            ...
            >
              ...
              Upload Video {autoTranscript && '& Generate Transcript'}
            </Button>
          </div>
        )}
      </CardContent>
